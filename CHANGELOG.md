...
## Session 30 — Mobile-lock width root cause, five palette themes, global font-size

### Context
This session picks up from an out-of-band session that produced a
`courtyard-pwa-mobile-lock` patch (the standalone-viewport detection,
`--vh-100`, and a JS `zoom`-based width correction) — that patch was never
logged here, so this entry covers both confirming/fixing that work and
the three new features requested this session.

### Part 1 — PWA mobile width, root cause
On-device testing showed the previous session's JS width fix
(`correctStandaloneViewportZoom()` in `pwa.js`, using
`window.visualViewport.scale` to set a counter-`zoom` on `<html>` plus a
JS-measured `--true-vw`) was the actual source of the "content squeezed
left, dead space on the right" bug, not a fix for it: the debug badge
captured `visualViewport.scale: 0.25`, `applied zoom (fix): 4`, and
`innerWidth: 1646` — 1646 is ~4x the device's real 412px width, i.e.
exactly the zoom factor the fix had just applied. The zoom write fed back
into the very `visualViewport` reading used to decide how much zoom to
apply, so it never converged — it kept re-asserting a 4x correction
against a problem it was itself causing.
- **Removed**: `correctStandaloneViewportZoom()`, `trueDeviceWidth()`,
  `window.__COURTYARD_APPLIED_ZOOM__`, `window.__COURTYARD_TRUE_VW__`
  (`pwa.js`); the `--true-vw` custom property and the
  `max-width:var(--true-vw); transform:translateZ(0);` rule (`styles.css`).
- **Replaced with**: `html.pwa-standalone{ width:100%; max-width:100vw; }`
  and `html.pwa-standalone body{ width:100%; max-width:100%; margin:0;
  overflow-x:hidden; }` — pure CSS, never reads a scale value back, so it
  can't self-reinforce the way the old mechanism did.
- The `--vh-100`/`100dvh` height fix from the earlier session is
  untouched — this was a width-only problem, per the brief.
- Debug badge (`pwa.js`) kept for one more round of on-device
  verification, with the now-removed zoom/`--true-vw` rows dropped and an
  `html computed width` row added alongside `body computed width` so the
  two can be compared directly.
- **Bug found and fixed in passing**: `service-worker.js`'s navigation
  handler had a stray extra `}` before its `catch` block (line ~249),
  which is a genuine syntax error — `node --check` failed on it. This
  would have broken the service worker's install/registration entirely,
  independent of anything else in this session. Fixed by removing the
  extra brace; not part of the original brief but couldn't be left in
  place once found.

### Part 2 — Five palette themes, new default
Replaced the three-theme dusk/night/dawn system with five palettes built
from the supplied five-color reference (Espresso Clay, Earth Brown,
Mushroom Beige, Warm Putty, Bone White). Each theme re-values the same
custom-property set (`--ink`/`--ink-deep`/`--midnight`/`--midnight-deep`,
`--ivory`/`--ivory-dim`/`--ivory-faint`, `--gold`/`--gold-bright`/
`--gold-soft`, `--candle`, `--line`/`--line-soft`) the app already reads
everywhere, so no component rule needed to change. Background/text pairs
were tuned (not used as flat swatch-to-background copies) to keep WCAG AA
contrast (~4.5:1+) for primary text and ~4.3:1+ for accent text across
every theme; secondary/muted text follows the same reduced-opacity
convention the original three themes already used. **Warm Putty is the
new default** (baked into `:root`, no `data-theme` attribute needed) —
chosen as the most reading-comfortable option that still feels warm
rather than clinical; Bone White is the lightest/highest-contrast option
for anyone who wants that instead.
- Added a new `--ink-deep-rgb` triplet per theme so `header{}`'s
  translucent background can be built with `rgba()` from the active
  theme's own color — the header was previously a **hardcoded near-black
  rgba**, invisible against the new light themes (dark nav text on a dark
  bar). This was a real bug the new themes would otherwise have exposed.
- `applyTheme()` (`app.js`) now validates against the five new keys and
  falls back to the default for anyone with an old `dusk`/`night`/`dawn`
  value already in `localStorage` — no migration prompt, just a quiet
  fallback.
- The rare "dawn" bird easter egg (`scheduleBird()`) now checks for the
  `bone` theme instead — its closest successor in the new palette — so
  the detail isn't silently dropped.
- Settings UI: the old three-dot picker is replaced by a `.theme-swatches`
  grid of five circular swatches, each showing its own palette color, a
  name label, and (when selected) a gold ring **and** a small check badge
  — selection is never color-only. New markup lives in its own `.card` in
  Settings, titled "Appearance."
- `manifest.webmanifest`'s `theme_color`/`background_color` and
  `index.html`'s `<meta name="theme-color">` updated to match Putty's
  `--ink-deep`, so the OS status bar/splash screen match the new default
  instead of the old dark palette.
- Left alone: the arrival-ritual decorative flourishes (pillars, mandala
  glow, insects, feather, and the bird SVG's stroke color), and the
  sound-panel "parchment" popup — these use fixed warm-ivory/beige tones
  independent of `--ivory`/`--ink` by original design, and are atmospheric
  rather than textual, so they were left as-is rather than folded into
  the theme system.

### Part 3 — Global font-size, three levels
Every `font-size` in `styles.css` already used `rem` (relative to
`html`, not `body`) except body's own base size (`18.5px`, absolute) —
so the entire scaling mechanism is two changes: `html{ font-size: calc(100%
* var(--font-scale)); }` (with `--font-scale:1` by default), and body's
base size expressed as `1.15625rem` (the same 18.5px at the default
scale) instead of a fixed px value. `html[data-font-size="small"]` sets
`--font-scale:0.92`; `html[data-font-size="large"]` sets `1.18`. No
`transform: scale()` anywhere — every rem-sized rule in the app (nav,
headings, chapter/verse text, buttons, labels, dialogs, journal,
meditation, journey, about, PWA banners, empty states) scales through
normal layout, so spacing reflows instead of clipping.
- `applyFontSize()` (`app.js`) mirrors `applyTheme()`'s shape: validates
  against `small`/`normal`/`large`, toggles `document.documentElement
  .dataset.fontSize`, persists via the existing `Storage` wrapper under a
  new `fontSize` key.
- Settings UI: a new `.font-size-control` pill with three "A" buttons at
  increasing glyph size (small → large), inspired by, not copied from,
  the Samsung font-size reference. The active option gets a filled
  background, bold weight, and a small dot — again, not color alone.
  Lives in its own "Reading / Text Size" card in Settings.

### Config / cache
- `config.json` and the `CONFIG` fallback in `app.js`: `defaultTheme`
  `"dusk"` → `"putty"`, added `defaultFontSize: "normal"`, version
  `2.1.0` → `2.2.0`.
- `service-worker.js`'s `CACHE_VERSION` bumped `v9` → `v10` — this
  session touches `index.html`, `styles.css`, `app.js`, `pwa.js`, and
  `config.json`, all precached app-shell files.

### What was intentionally left alone
- Routing, chapter loading, ambience, journal, journey/stats logic —
  nothing in `app.js` outside the theme/font-size/easter-egg lines above
  was touched.
- `reduceMotion` and all other existing Settings rows — untouched, still
  in their original positions above the two new cards.
- The arrival ritual and sound-panel decorative colors (see Part 2).
- App icon artwork (`assets/icons/*.png`) — still reflects the old dark
  palette; regenerating it to match Putty would need real image tooling,
  out of scope here.

### Verification performed
- `node --check` passes for `app.js`, `pwa.js`, and `service-worker.js`
  (the last one failed before the brace fix above).
- Manual WCAG contrast calculations (relative luminance / contrast ratio
  formula) for primary and accent text against every background stop in
  all five themes — all ≥4.3:1, most well above.
- Verified every `font-size` in `styles.css` is `rem`-based (`grep` for
  literal `px` font-sizes returned nothing outside the one body rule that
  was converted).
- Verified no remaining references to the removed `.theme-dot`/
  `.theme-options` classes or the old theme keys (`dusk`/`night`/`dawn`)
  outside the intentional one-line easter-egg remap.
- HTML tag counts and CSS brace counts balanced after all edits.

### What remains — please test on your phone
- The actual width fix (Part 1) could only be verified by static
  reasoning and `node`/brace-balance checks in this environment — there's
  no live device here. Please reinstall the PWA and check the debug badge
  again: `body computed width` and `html computed width` should now both
  read close to your phone's real screen width (e.g. ~412px), not 980 or
  a multiple of your device width.
- Large font size on a 360px-wide phone: check nav, chapter rows, and
  settings rows for any text touching the screen edge or wrapping
  awkwardly — the scale (1.18x) was chosen to be safe but hasn't been
  seen on a real narrow screen.
- All three font sizes and all five themes should survive closing and
  reopening the installed app (both now persist via the same `Storage`
  wrapper `theme` already used).
- Once width and themes are both confirmed solid on-device, the
  TEMPORARY debug badge in `pwa.js` (search for `mountPwaDebugBadge`) can
  be deleted per its own comment.
